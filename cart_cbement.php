...
<html>
    ...
    <body>
        ...
        foreach ($_POST as $productID => $productQty) {
            ...
            if (is_numeric($productQty) && $productQty > 0 && !isset($remove[$productID])){
                $cart[$productID] = $productQty;
            } else if ($productQty == 0 || isset($remove[$productID])){
        ...
        if (!$_SESSION['numItems']) {   // cart is empty
            ...
            $query = "SELECT * FROM products WHERE prodid IN ($prodIDStr);
                      ORDER BY category, prodid";
        ...
        <br><br>
        <form action="cart_cbement.php" method="post">
